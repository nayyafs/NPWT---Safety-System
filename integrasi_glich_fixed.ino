...
void updateTimerDisplay() {
    if (lockState) {
        // When locked, update timer at the top of screen
        ...
        if (timerRunning) {
            ...
            // Ensure lock message stays visible
        ...
        // Original timer display for unlocked state
...
void showLockMessage() {
    // Save the current screen content by not clearing it
    ...
    // Draw the lock message in the center of the screen
    ...
    // If timer is running, display it at the top of the screen
    if (timerRunning) {
        // Clear the top area for timer
        tft.fillRect(0, 0, SCREEN_WIDTH, 50, TFT_WHITE);
        
        // Draw timer in larger text at the top
        tft.setTextColor(TIMER_COLOR, TFT_WHITE);
        tft.setTextSize(3);
        unsigned long remainingTime = (timerDuration - (millis() - startTime)) / 1000;
        if (remainingTime > 0) {
...
void loop() {
    ...
    // Button 1 controls both pump and timer
    if (lastState_1 == HIGH && currentState_1 == LOW) {
        if (!pumpState) {
            ...
            pumpState = true;
            greenState = true;
            startTimer();  // Start the timer when pump is turned on
        } else  {
            // Turn off pump and stop timer
            digitalWrite(LED_GREEN, LOW);
            pumpState = false;
            greenState = false;
            stopTimer();   // Stop the timer when pump is turned off
        }
    ...
    if (Serial.available() > 0){
      String bmp_pressure = Serial.readString(); // ini dari sensor bmp
      int rt_pressure = bmp_pressure.toInt();
      control_buzzer(rt_pressure);
      }

    if(greenState){  //green state: kondisi led hijau di safety system
      digitalWrite(LED_GREEN, HIGH);
      }
    else{
      digitalWrite(LED_GREEN, LOW);
    ...
    // Only handle touchscreen input if screen is not locked
    if (!lockState) {
    ...
    // Update timer display if running (this should work even when locked)
    if (timerRunning) {
