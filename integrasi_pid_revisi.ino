...
// Pressure control
int targetPressure = 120;
const int pressureStep = 10;
const int minPressure = 80;
...
Button minusBtn = {150, ROW_P, BUTTON_WIDTH, BUTTON_HEIGHT, "-"};
Button plusBtn = {150 + BUTTON_WIDTH + BUTTON_SPACING, ROW_P, BUTTON_WIDTH, BUTTON_HEIGHT, "+"};
Button timerMinusBtn = {150, ROW_Y, BUTTON_WIDTH, BUTTON_HEIGHT, "-"};
Button timerPlusBtn = {150 + BUTTON_WIDTH + BUTTON_SPACING, ROW_Y, BUTTON_WIDTH, BUTTON_HEIGHT, "+"};
...
void updateTimerDisplay() {
    if (lockState) {
        ...
        tft.fillRect(180, 180, 200, 30, BACKGROUND);
        ...
        if (timerRunning) {
            ...
            if (remainingTime > 0) {
                ...
                tft.drawString(timeText, 180, 180, 1);
            } else {
                timerRunning = false;
                tft.drawString("Time's up!", 180, 180, 1);
                greenState = false;
                pumpState = false;
                digitalWrite(LED_RED, LOW);
                analogWrite(pumpPin, 0);
...
void loop() {
    ...
    input = (bmp.readPressure() / 100.0F) + 33.33; // Read pressure in hPa
    //input = (bmp.readPressure() / 100.0F);

    // Convert pressure to mmHg
    float pressure_mmHg = input * 0.750062;
    float selisih = pressure_mmHg - (atm/1.33);
   ...
   /* Serial.print("Pressure = ");
    ...
    if (pumpState) {
        ...
        if((error > 0.2*setpoint || error < -0.2*setpoint) && count > 500){
